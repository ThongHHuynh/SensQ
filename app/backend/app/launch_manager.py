...
class LaunchManager:
    ...
    async def _find_existing_launch_pids(self) -> list[int]:
        process = await asyncio.create_subprocess_exec(
            "pgrep",
            "-f",
            f"ros2 launch {ROBOT_LAUNCH_PACKAGE} {ROBOT_LAUNCH_FILE}",
    ...
    async def start_mobile_base(self) -> dict:
        ...
        await save_event(
            "launch",
            f"Started {ROBOT_LAUNCH_PACKAGE}/{ROBOT_LAUNCH_FILE}",
        )
    ...
    async def stop(self) -> dict:
        ...
        if stopped_external:
            ...
            await save_event(
                "launch",
                f"Stopped {ROBOT_LAUNCH_PACKAGE}/{ROBOT_LAUNCH_FILE}",
            )
        await ws_manager.broadcast(snapshot)
        return robot_state.get_snapshot()
    ...
    async def _read_logs(self) -> None:
        ...
        if return_code != 0:
            ...
            await save_event(
                "launch",
                f"{ROBOT_LAUNCH_PACKAGE}/{ROBOT_LAUNCH_FILE} exited with code {return_code}",
                "error",
            )
        else:
            snapshot = robot_state.set_launch_state("stopped")
            await save_event(
                "launch",
                f"{ROBOT_LAUNCH_PACKAGE}/{ROBOT_LAUNCH_FILE} exited",
            )
