...
DATABASE_URL="${DATABASE_URL:-sqlite+aiosqlite:///$ROOT_DIR/data/sensq.db}"
...
Or run SensQ on another port:
  ...
  BACKEND_PORT=8001 FRONTEND_PORT=5201 ./app/dev.sh
...
require_file "$BACKEND_DIR/.venv/bin/python" "Backend virtualenv missing. Run: cd app/backend && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt"
require_file "$UI_DIR/node_modules" "Frontend dependencies missing. Run: cd app/ui && npm install"

if [[ "$DATABASE_URL" == postgresql* ]] && ! python3 - <<'PY' >/dev/null 2>&1
import socket
...
Fix one of these:
  ...
  3. Or run with your real database URL:
     ...
     DATABASE_URL="postgresql+asyncpg://USER:PASSWORD@localhost:5432/DB_NAME" ./app/dev.sh
...
(
  ...
  exec "$BACKEND_DIR/.venv/bin/python" -m uvicorn app.main:app --reload --host "$BACKEND_HOST" --port "$BACKEND_PORT"
) &
BACKEND_PID="$!"
