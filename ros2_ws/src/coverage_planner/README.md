...
ROS 2 safe-map coverage planner for mobile robots.
...
- `coverage_executor_node`: lifecycle Nav2 `FollowWaypoints` executor with start/pause/resume/cancel services.
...
ros2 launch coverage_planner production_path.launch.py use_sim_time:=false
...
```bash
python3 -m pytest src/coverage_planner/unit_test
```
